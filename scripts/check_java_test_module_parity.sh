...
if [ -f "tests/layered_issue_test_parity.md" ]; then
    layered_issue_status="$(awk -F': ' '/^- status:/ {print $2; exit}' tests/layered_issue_test_parity.md)"
    layered_issue_java_tests="$(awk -F': ' '/^- java @Test total:/ {print $2; exit}' tests/layered_issue_test_parity.md)"
    layered_issue_rust_tests="$(awk -F': ' '/^- rust #\[test\] total:/ {print $2; exit}' tests/layered_issue_test_parity.md)"
fi

{
    ...
    echo '- For layered issue method-level parity, use `tests/layered_issue_test_parity.md`.'
    echo
