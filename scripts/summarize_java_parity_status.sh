...
mkdir -p "$(dirname "$OUTPUT")"
{
  ...
  if [ "$java_candidate_exists" = "yes" ] && [ "$candidate_check_status" = "ready" ]; then
    ...
    if [ "$candidate_matches_baseline" = "yes" ]; then
      ...
      echo "- re-run compare in \`java_compare_mode=both\` with desired parity gates when Rust parity inputs change."
    else
      echo "- promote candidate with \`sh scripts/update_java_parity_baseline.sh \"$JAVA_BASELINE_CANDIDATE_FILE\" \"$JAVA_BASELINE_FILE\"\`"
