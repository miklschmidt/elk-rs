...
REPORT_FILE="${1:-tests/algorithm_name_parity.md}"
STRICT_MODE="${ALGORITHM_NAME_PARITY_STRICT:-false}"

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/algorithm-name-parity.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT INT TERM

mkdir -p "$(dirname "$REPORT_FILE")"

java_file="$tmp_dir/java_algorithm_name.tsv"
rust_map_file="$tmp_dir/rust_algo_map.tsv"
rust_file="$tmp_dir/rust_algorithm_name.tsv"
...
mismatch_file="$tmp_dir/name_mismatches.tsv"
...
# Rust: id -> name from LayoutAlgorithmData::new + set_name + register_* calls.
(rg --files "$RUST_SOURCES_ROOT" -g '*.rs' -g '!**/tests/**' || true) \
    | while IFS= read -r file; do
        ...
        awk -v map_file="$rust_map_file" '
            ...
            function parse_string(text,    value) {
                if (!match(text, /"[^"]+"/)) {
                    return ""
                }
                value = substr(text, RSTART + 1, RLENGTH - 2)
...

            {
                ...
                target_var = ""
                if (line ~ /^[[:space:]]*\.[[:space:]]*set_name[[:space:]]*\(/) {
                    target_var = chain_var
                } else if (line ~ /[A-Za-z_][A-Za-z0-9_]*[[:space:]]*\.[[:space:]]*set_name[[:space:]]*\(/) {
                    target_var = line
...

{
    ...
    echo "- name mismatches: $mismatch_count"
    echo
    echo "## Name Mismatches (id | java | rust)"
    if [ -s "$mismatch_file" ]; then
        while IFS="$(printf '\t')" read -r id java_name rust_name; do
            echo "- $id | $java_name | $rust_name"
        done < "$mismatch_file"
    else
        echo "- none"
    fi
...
if [ "$STRICT_MODE" = "true" ] && [ "$status" != "ok" ]; then
    echo "algorithm name parity drift detected (strict mode): $REPORT_FILE" >&2
    exit 1
fi

echo "wrote algorithm name parity report: $REPORT_FILE"
